...
def build_unweighted_graph(nodes, adjacency_counts):    #changed the name
    G = nx.Graph()  
...
def compute_distance_matrix(nodes, adjacency_counts, distance_mode="inverted"):
    ...
    for (a, b), count in adjacency_counts.items():  # Replace [] with adjacency_counts.items()
        if a in node_index and b in node_index:  # Replace False with condition: a in node_index and b in node_index
            count_matrix[node_index[a], node_index[b]] = count  # Add the count to the appropriate matrix position
    ...
    np.fill_diagonal(count_matrix, 0)  # Replace this line
...
def tokenize_text(text, char_like=["'"]):
    ...
    # STEP 2: Process each character in the text
        ...
        # STEP 3: Handle alphabetic characters (letters)
        ...
        # STEP 4: Handle punctuation (comma and period)
        ...
        # NOTE: "Flush current_word" means:
        #   1. Check if current_word has content (if current_word:)
        #   2. Join the characters: ''.join(current_word)
        #   3. Add to tokens list
        #   4. Reset current_word to empty list: current_word = []
...
# STEP 1: Extract only words (non-punctuation) for threshold calculation
    ...
    new_tokens = [rare_token 
                  if token in rare_set 
                  else token 
                       for token in tokens]  # Replace this line
...
def get_text_adjacencies(tokens):
    ...
    for i in range(len(tokens) - 1):  # Replace 0 with the correct range
        ...
        if current_token != next_token:  # Replace False with the correct condition
           ...
           pair = (current_token, next_token)  # Replace this line with your code
...
def get_spatial_adjacencies(quantized_image):
    ...
    height, width, channels = quantized_image.shape  # Replace this line
    ...
    directions = [(-1,0), (1,0), (0,-1), (0,1)]  # Replace this line
    ...
    for row in range(height): # Replace 0 with correct range
        for col in range(width):  # Replace 0 with correct range
            ...
            current_color = tuple(quantized_image[row, col])  # Replace this line

            print(f"Pixel ({row},{col}): color={current_color}")  # Debug helper (optional)

            # STEP 6: Count this color's frequency
            # HINT: Increment color_frequencies[current_color]
            color_frequencies[current_color] += 1  # Replace this line

            # STEP 7: Check all 4-connected neighbors
            # HINT: Loop through directions: for di, dj in directions:
            for di, dj in directions:  # Replace [] with directions

                # Calculate neighbor coordinates
                # HINT: ni = i + di (neighbor row)
                # HINT: nj = j + dj (neighbor column)
                ni = row + di # Replace this line
                nj = col + dj  # Replace this line
                ...
                if 0 <= ni < height and 0 <= nj < width:  # Replace False with correct condition
                    ...
                    if current_color != neighbor_color:  # Replace False with correct condition
                        ...
                        adjacency_counts[(current_color, neighbor_color)] += 1  # Replace this line
    ...
    unique_colors = sorted(color_frequencies.keys(), key=lambda x: (-color_frequencies[x], x))  # Replace this line
...
class TestUnifiedNetworks(unittest.TestCase):
    ...
    def test_compute_distance_matrix_inverted_mode(self):
        ...
        expected_count = np.array([[0, 3], [3, 0]])
        np.testing.assert_array_almost_equal(count_matrix, expected_count, decimal=1)
        #self.fail("Test not implemented yet - replace this with your assertions")
        ...
        max_count = 3.0
        expected_distance = [[0, 1], [1,0]]
        np.testing.assert_array_almost_equal(distance_matrix, expected_distance, decimal=1)
        ...
        np.testing.assert_array_equal(np.diag(distance_matrix), [0,0])
        np.testing.assert_array_equal(np.diag(distance_matrix), [0, 0])
    ...
    def test_tokenize_text_basic(self):
        ...
        text = "Hello, world! How are you?"  # Replace this line
        ...
        expected = ["hello", ",", "world", "how", "are", "you"]  # Replace this line
        ...
        text_numbers = "abc123def"
        for sentence in text_numbers:
            if sentence.isdigit():
                text_numbers = text_numbers.replace(sentence, "")   #looping through text_numbers, checking for digits, if there is a digit this is removed.
        
        output = tokenize_text(text_numbers)
        expected = ["abcdef"]
    ...
    def test_get_spatial_adjacencies_basic(self):
        ...
        # Example: [[[0,0,0], [1,1,1]],
        #           [[2,2,2], [3,3,3]]]
        quantized_image = np.array([[[0,0,0], [255,0,0]],
                                    [[0,255,0], [0,0,255]]])  # Replace with 4 different colors
        ...
        for color, count in color_frequencies.items():
            self.assertEqual(count, 1, {color})
        ...
        self.assertEqual(total_adjacencies, 8)
        ...
        self.assertGreater(adjacency_counts[((0,0,0), (255,0,0))], 0)  
        self.assertGreater(adjacency_counts[((0,0,0), (0,255,0))], 0)  
        self.assertGreater(adjacency_counts[((255,0,0), (0,0,255))], 0)  
        self.assertGreater(adjacency_counts[((0,255,0), (0,0,255))], 0)  
